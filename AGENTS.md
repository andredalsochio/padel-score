...
> - Global preferences are defined in `~/.trae/rules/user_rules.md`.  
...
> **Flutter SDK Version:**  
> Always use the latest stable Flutter release available for macOS on Apple Silicon (ARM64).  
> As of November 2025, this project targets Flutter **3.35.x stable** with Dart **3.6+**, managed via FVM.  
...
To keep UIs scalable and maintainable, all new and refactored screens MUST follow a modular widget architecture.

- Split large screens (e.g., `HomeView`/`HomeScreen`) into small, reusable sub-widgets.
- Place sub-widgets under feature folders like `lib/features/<feature>/widgets/` or `components/`.
...
- Use relative imports (e.g., `import '../widgets/quick_actions.dart';`) rather than package-style self-imports.
- Naming conventions: files `snake_case.dart`, classes `PascalCase`, members/functions `camelCase`.

## Modern Flutter API Usage (Flutter 3.35+)

Adopt current Material 3 and Color APIs and avoid deprecated approaches.

- Stop using `Color.  (...)`. Prefer `Color.withValues(alpha: <0.0–1.0>)` for alpha changes.
- Prefer `AnimatedOpacity`, `FadeTransition`, and Material motion widgets for opacity/motion over manual color math.
- Use `Theme.of(context).colorScheme` for colors; avoid hardcoded ARGB unless necessary.
- Favor Material 3 components and transitions; configure motion with `ThemeData` to ensure consistency.

## Code Generation & Structure Guidelines

Ensure consistent, clean outputs when generating code.

- One widget per file to improve discoverability and reuse.
- Use relative imports across the project; avoid absolute `package:` self-imports.
- Keep feature-based structure (MVVM with Provider) and avoid mixing concerns.
- Follow SOLID and keep functions small and purposeful (generally <20 lines).
- Do not modify unrelated code during refactors; keep patches surgical.
...
This project integrates three Model Context Protocol (**MCP**) servers that work together within the same workspace environment.  
These MCPs are critical components of the `Padel App`, and the IDE must actively use them to provide intelligent context, data synchronization, and backend interaction.

**Configured MCP Servers**

- **`pg` (PostgreSQL MCP Server)**  
  - Manages database schema, tables, and direct SQL operations.  
  - Connected to the Supabase-hosted PostgreSQL instance for this project.

- **`supabase` (Supabase MCP)**  
  - Provides API access, authentication, and storage integration.  
  - Acts as the middleware for real-time data sync and client communication.

- **`context7` (Upstash Context7 MCP)**  
  - Handles AI context synchronization and memory management between the workspace and Codex.  
  - Keeps project context shared across IDE and AI assistants.

All three MCPs are interdependent and must remain active.  
When executing Codex or Trae commands, the IDE should use these MCP servers automatically to ensure seamless operation between frontend (`Flutter`) and backend (`PostgreSQL`/`Supabase`).

### 🧠 AI Workflow and Progress Tracking

All AI-driven implementations must follow a structured two-phase workflow to ensure clarity, traceability, and continuity across sessions.

#### 1. Implementation Plan
Before writing any code, the AI must prepare a concise plan that includes:
- **Scope:** What the feature or change covers.
- **Approach:** The strategy, architecture impact, and affected modules.
- **Dependencies:** Libraries, MCP servers, or APIs involved.
- **Steps:** A checklist of sequential actions.

Output this plan in Markdown with checkboxes.

#### 2. Execution Plan
Once the implementation plan is approved, the AI executes it step by step.
- After completing each step, mark it as ✅ in the shared log.
- Avoid skipping steps or combining multiple in one action without user confirmation.

#### 3. Progress Recording
The AI must record all progress in a workspace file named: